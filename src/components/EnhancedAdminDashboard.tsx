...
import {
  ...
  CheckSquare, Square, Trash2, CheckCircle, X
} from 'lucide-react';
...
export default function EnhancedAdminDashboard() {
  ...
  const handleBulkDelete = async () => {
    ...
    await supabase.from('admin_notes').delete().in('conversation_id', ids);
    await supabase.from('analytics_events').delete().in('conversation_id', ids);
    await supabase.from('conversation_tags_junction').delete().in('conversation_id', ids);
