...
export default function ResourceBrowser() {
  ...
  const searchResources = useCallback(async () => {
    ...
    if (error) {
      ...
      if (searchQuery.trim()) {
        // Escape special characters to prevent SQL injection via .or() interpolation
        ...
        let fallback = supabase
          ...
          .select('id, title, content, category, city, state, zip_code, phone, website, tags')
          .eq('is_active', true)
          .or(`title.ilike.${pattern},content.ilike.${pattern},city.ilike.${pattern}`)
          .order('title', { ascending: true })
          .limit(50);
  ...
  }, [searchQuery, selectedCategory, selectedCity, searchResources]);
        ...
        {loading ? (
          ...
          <div className="divide-y divide-slate-100">
            {entries.map((entry) => {
              ...
              return (
                <div key={entry.id} className="w-full text-left">
                  <button
                    onClick={() => setExpandedId(isExpanded ? null : entry.id)}
                    aria-expanded={isExpanded}
                    aria-controls={`resource-panel-${entry.id}`}
                  ...
                  {isExpanded && (
                    <div id={`resource-panel-${entry.id}`} className="px-3 sm:px-4 pb-3 space-y-2">
